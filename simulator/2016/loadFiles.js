...
function loadRoom(name){
	...
	GDownloadUrl(name+".json", function dispData(fdata, statusCode){
		...
		while(name.charAt(name.length-1)!="\\".charAt(0)){
			name = name.substring(0,name.length-1);
		}
		var mat_floor = new THREE.MeshBasicMaterial({map:THREE.ImageUtils.loadTexture(name+obj.texture[1])});
		camera_r=obj.camera_r;
		camera_theta=obj.camera_theta;
		camera_phi=obj.camera_phi;

		// 壁の質量
		var wallmass = 1000000000000.0;
		var room_x = obj.wX;
		var room_y = obj.wY;
		var room_z = obj.wZ;
		...
		var mat_wall = [];
		for (var i=0;i<6;i++){
			if (obj.useTexture[2+i]==1)
				mat_wall[i]=new THREE.MeshPhongMaterial({map:THREE.ImageUtils.loadTexture(name+obj.texture[i]),transparent:true, opacity:obj.opacity[i], side:THREE.DoubleSide});
			else
				mat_wall[i]=new THREE.MeshLambertMaterial({color:obj.color[i],transparent:true, opacity:obj.opacity[i], side:THREE.DoubleSide});
		...
		var t_wall=[];
		var wallGeometry=[
			new THREE.BoxGeometry(room_x+room_t*2, room_y+room_t*2, room_t),//天井
			new THREE.BoxGeometry(room_x+room_t*2, room_y+room_t*2, room_t),//床
			new THREE.BoxGeometry(room_t, room_y+room_t*2, room_z),
			new THREE.BoxGeometry(room_t, room_y+room_t*2, room_z),
			new THREE.BoxGeometry(room_x, room_t, room_z),
			new THREE.BoxGeometry(room_x, room_t, room_z)
		];
		var wallPosition=[
			new THREE.Vector3(0, 0 , room_z+room_t/2),
			new THREE.Vector3(0, 0 , -room_t/2),
			new THREE.Vector3(-room_x/2-room_t/2, 0 , room_z/2),
			new THREE.Vector3( room_x/2+room_t/2, 0 , room_z/2),
			new THREE.Vector3(0, -room_y/2-room_t/2 , room_z/2),
			new THREE.Vector3(0,  room_y/2+room_t/2 , room_z/2)
		];
		var t_wallGeometry=[
		...
		room._physijs.mass=wallmass * 6;
		room._physijs.friction=obj.friction;
		...
		name2 +="funitures\\";
//		var folderRef=new Folder(name);
		var inp = document.getElementById('fileElementId');
		console.log(inp.files);
		for (var i = 0; i < inp.files.length; ++i) {
 			var name3 = inp.files.item(i).name3;
			alert("here is a file name: " + name3);
		}
		console.log(name);
		console.log(name2);
		console.log(inp.file);

		for(var i=0;i<obj.furnitures.length;i++){
			console.log(i,"a");
		}
		scene.add(room);
